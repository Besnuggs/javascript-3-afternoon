...
let numObj = [first,second,third];
let smallestValue = 500000000000000000000;
for (let i = 0; i < numObj.length ; i++){
  if (numObj[i] < smallestValue) {
      smallestValue = numObj[i]
  }
...
let numberGroups = ({a,b,c}) => {
  ...
  for (let i = 0 ; i < newArr.length; i++){
    if (newArr[i].length > startingLength.length){
      startingLength = newArr[i]
    }
  }
